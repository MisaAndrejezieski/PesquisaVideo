...
def abrir_edge_sem_perfil():
    """Abre o Edge em modo temporário (sem login nem perfil fixo)."""
    edge_path = localizar_edge()
    if not edge_path:
        return False

    temp_profile = tempfile.mkdtemp()  # Cria uma pasta temporária para o perfil
    subprocess.Popen(
        [
            edge_path,
            "--new-window",
...
janela = tk.Tk()
janela.title("Automação de Vídeos - PyAutoGUI + Edge (Sem Usuário)")
